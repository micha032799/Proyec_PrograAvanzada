...
namespace API_PA.Controllers
{
    ...
    public class CarritoController : ApiController
    {
        ...
        [HttpPost]
        [Route("PagarCarrito")]
        public string PagarCarrito(TCarrito tCarrito)
        {
            using (var context = new PAEntities())
            {
                return context.Pagar_CarritoSP(tCarrito.ConUsuario).FirstOrDefault();
            }
        }
        ...
        public List<TMaestro> ConsultarFacturas(long q)
        {
            ...
            {
                context.Configuration.LazyLoadingEnabled = false;
                return (from x in context.TMaestro where x.ConUsuario == q select x).ToList();
            }
        }
